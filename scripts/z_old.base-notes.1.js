class BaseNotes {
  constructor(controls=new Object) {
    ...
    // }
    // this.controls.listItems.style.width = 600 + getScrollbarWidth();
  ...
  init() {
    ...
    this.validator.addRule(function () {
      return !(this.controls.title.value.trim().length === 0);
    }.bind(this));

    // this.validators.add(function () {
    //   var hasError = (this.controls.title.value.trim().length === 0);

    //   if(hasError) {
    //     if (!this.controls.title.classList.contains('required')) {
    //       this.controls.title.classList.add('required');
    //     } else {
    //       this.controls.title.style.animation = 'none';
    //       this.controls.title.offsetHeight; // trigger reflow
    //       this.controls.title.style.animation = null;
    //     }
    ...
    // this.validators.cancelEvent = function(e) {
    //   console.log('----' + e.type);

    //   if ((e.target !== this.controls.title || e.key === "Tab") &&
    //     !this.validators.validateTitle(false)) {
    //     e.preventDefault();
    //     e.stopPropagation();
    //   }
    // }.bind(this);

    // this.validators.validateTitle = function (checkOnly=false) {
    //   var hasError = (this.controls.title.value.trim().length === 0);

    //   if (hasError && !checkOnly) {
    //     if (!this.controls.title.classList.contains('required')) {
    //       this.controls.title.classList.add('required');
    //     } else {
    //       this.controls.title.style.animation = 'none';
    ...
    if (!this.editMode) {
      ...
      this.editMode.save = function () {
        if (this.editMode.titleInput.value.trim().length === 0) {
          if (!this.editMode.titleInput.classList.contains('required')) {
            this.editMode.titleInput.classList.add('required');
          } else {
            this.editMode.titleInput.style.animation = 'none';
            this.editMode.titleInput.offsetHeight; // trigger reflow
            this.editMode.titleInput.style.animation = null;
          }
  ...
  titleChanged(e) {
    if (!this.validator.valid()) {
      ...
      // this.controls.description.disabled = true;
      // this.controls.delete.disabled = true;




      // document.addEventListener('mousedown', this.validators.cancelEvent);
      // document.addEventListener('keydown', this.validators.cancelEvent);

      return;
    }

    // document.removeEventListener('mousedown', this.validators.cancelEvent, { passive: true });
    // document.removeEventListener('keydown', this.validators.cancelEvent, { passive: true });
